...
const { eq, desc, and, sql, gte,lte, inArray } = require('drizzle-orm');
...
class IoTRepository {
  ...
  async createActuator(actuatorData) {
    const result = await db.insert(actuators).values(actuatorData).returning();
    return result[0];
  }

  async deleteSensor(sensorId) {
    await db.delete(readings).where(eq(readings.sensorId, sensorId));
    const result = await db.delete(sensors).where(eq(sensors.id, sensorId)).returning();
    return result[0];
  }

  async deleteActuator(actuatorId) {
    await db.delete(actuatorLogs).where(eq(actuatorLogs.actuatorId, actuatorId));
    const result = await db.delete(actuators).where(eq(actuators.id, actuatorId)).returning();
    return result[0];
  }
   async getSensorReadings(sensorId, limitCount = 10) {
    return await db.select()
        .from(readings)
  ...
  async getActuatorLogs(actuatorId, limitCount = 10) {
    return await db.select()
        .from(actuatorLogs)
        .where(eq(actuatorLogs.actuatorId, actuatorId))
        .orderBy(desc(actuatorLogs.timestamp))
        .limit(limitCount);
  }
 async getSensorChartData(sensorId, startDate, endDate) {
    return await db.select({
    ...
    .where(and(
        ...
        gte(readings.timestamp, startDate),
        lte(readings.timestamp, endDate)
    ))
    .orderBy(readings.timestamp); // Від найстарішого до найновішого (зліва направо для графіка)
  }
